...
namespace mtk {
    ...
    public:
        ...
        void Fin();
        //callbacks
        void OnDestroy(grpc_error_handle error);
        void OnAccept(grpc_endpoint *tcp, grpc_pollset *accepting_pollset,
                      grpc_tcp_server_acceptor *acceptor);
    protected:
        //trampoline
        static void _OnDestroy(void *arg, grpc_error_handle error) {
            ((HttpServer *)arg)->OnDestroy(error);
        }
        static void _OnAccept(void *server,
                              grpc_endpoint *tcp, grpc_pollset *accepting_pollset,
                              grpc_tcp_server_acceptor *acceptor) {
            ((HttpServer *)server)->OnAccept(tcp, accepting_pollset, acceptor);
        }
        //handler adding
        void AddHandler(int port, Callback cb);
        bool DoListen(int port);
        ...
        //io
        std::vector<gpr_mu*> polling_mus_;
    ...
    public:
        ...
        static void Fin();
        static bool Start(const std::string &root_cert);
        static void Stop();
