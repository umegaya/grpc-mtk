...
bundle: proto
	- mkdir -p build/osx
	cd build/osx && cmake -DCMAKE_TOOLCHAIN_FILE=$(PROJECT_ROOT)/toolchain/bundle.cmake $(PROJECT_ROOT) && make

ios: proto
	- mkdir -p build/ios.v7
	- mkdir -p build/ios.64
	- mkdir -p build/ios
	cd build/ios.v7 && cmake -DCMAKE_TOOLCHAIN_FILE=$(PROJECT_ROOT)/toolchain/ios.cmake -DIOS_ARCH=armv7 $(PROJECT_ROOT) && make
	cd build/ios.64 && cmake -DCMAKE_TOOLCHAIN_FILE=$(PROJECT_ROOT)/toolchain/ios.cmake -DIOS_ARCH=arm64 $(PROJECT_ROOT) && make
...
android: proto
	...
	cd build/android.v7 && cmake -DCMAKE_TOOLCHAIN_FILE=$(PROJECT_ROOT)/toolchain/android.cmake -DANDROID_ABI="armeabi-v7a" -DANDROID_NATIVE_API_LEVEL=android-21 -DANDROID_STL=c++_static $(PROJECT_ROOT) && make
	cd build/android.64 && cmake -DCMAKE_TOOLCHAIN_FILE=$(PROJECT_ROOT)/toolchain/android.cmake -DANDROID_ABI="arm64-v8a" -DANDROID_NATIVE_API_LEVEL=android-21 -DANDROID_STL=c++_static $(PROJECT_ROOT) && make
